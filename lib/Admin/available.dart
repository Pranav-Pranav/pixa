...
Available({required this.books});
  @override
  State<Available> createState() => _AvailableState(books: books);
}

class _AvailableState extends State<Available> {
  @override
  List<Map<String,dynamic>>books;
  _AvailableState({required this.books});

  Widget build(BuildContext context) {
    return Scaffold(
    body:
    ListView(
      children: books.map((book) => book["status"]=="available"?BookTile(book: book):SizedBox(height: 0)).toList(),
    ));
  }
}
